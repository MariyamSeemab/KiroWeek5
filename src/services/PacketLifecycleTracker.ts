...
import { RoutingPath, StationInfo } from '../models';
...
export class PacketLifecycleTracker {
  private activeTraces: Map<string, PacketTrace> = new Map();
  ...
  simulatePacketProgress(packetId: string, delayFactor: number = 0): PacketTrace | null {
    ...
    if (trace.currentHop === 1) trace.overallStatus = 'created';
    else if (trace.currentHop === 2) trace.overallStatus = 'collecting';
    else if (trace.currentHop === 3) trace.overallStatus = 'sorting';
    else if (trace.currentHop === 4) trace.overallStatus = 'delivering';
    else if (trace.currentHop > 4) trace.overallStatus = 'delivered';
...
╰─────────────────────────────────────────────────────────────────╯`;
  ...
  private generatePacketId(marker: string): string {
  ...
  private getHopStatus(trace: PacketTrace, hopNumber: number): string {
