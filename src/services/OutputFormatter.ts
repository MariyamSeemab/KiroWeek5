...
import { RoutingPath, ParsedMarker, ValidationError } from '../models';
...
export class OutputFormatter {
  ...
  formatRoutingPath(route: RoutingPath, format: 'terminal' | 'json' = 'terminal'): FormattedOutput {
  ...
  formatValidationErrors(errors: ValidationError[], format: 'terminal' | 'json' = 'terminal'): FormattedOutput {
    const timestamp = new Date().toISOString();
    ...
    const body = errors.map(error => 
      `❌ ${error.component.toUpperCase()}: ${error.message}`
    ).join('\n');
  ...
  formatReliabilityMetrics(route: RoutingPath, format: 'terminal' | 'json' = 'terminal'): FormattedOutput {
    const timestamp = new Date().toISOString();
  ...
  private formatTerminalReliabilityMetrics(metrics: any): string {
    const lines = [
    ...
    const activeFactors = metrics.systemConfidence.degradationFactors
      .filter((factor: any) => factor.isActive);
    ...
    metrics.complexityScore.hops.forEach((hop: any, index: number) => {
    ...
    if (metrics.alternateRoutes.length > 0) {
      ...
      metrics.alternateRoutes.forEach((alt: any, index: number) => {
  ...
  private formatTerminalRoutingPath(route: RoutingPath): string {
    const lines = [
