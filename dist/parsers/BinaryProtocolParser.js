...
exports.BinaryProtocolParser = void 0;
const fs = __importStar(require("fs/promises"));
const models_1 = require("../models");
class BinaryProtocolParser {
    constructor(protocolFilePath = '.kiro/binary_protocol.md') {
        this.protocolFilePath = protocolFilePath;
    }
    async parseProtocolFile(filePath) {
    ...
    extractStationMappings(content) {
    ...
    extractSymbolDefinitions(content) {
        ...
        for (const block of symbolBlocks) {
            ...
            let destinationType = models_1.DestinationType.INDUSTRIAL_ESTATE;
            let description = '';
            let examples = [];
    ...
    extractColorRules(content) {
        ...
        for (const block of colorBlocks) {
            ...
            let areaType = '';
            let priority = models_1.PriorityLevel.STANDARD;
            let hexCode = '';
            let description = '';
            for (const line of lines) {
    ...
    extractStationAliases(content, stationCode) {
    ...
    parseDestinationType(typeStr) {
        switch (typeStr.toLowerCase()) {
            case 'industrial estate':
                return models_1.DestinationType.INDUSTRIAL_ESTATE;
            case 'residential chawl':
                return models_1.DestinationType.RESIDENTIAL_CHAWL;
            case 'commercial complex':
                return models_1.DestinationType.COMMERCIAL_COMPLEX;
            case 'government office':
                return models_1.DestinationType.GOVERNMENT_OFFICE;
            case 'educational institute':
                return models_1.DestinationType.EDUCATIONAL_INSTITUTE;
            default:
                return models_1.DestinationType.INDUSTRIAL_ESTATE;
        }
    }
    parsePriorityLevel(priorityStr) {
        switch (priorityStr.toLowerCase()) {
            case 'urgent':
                return models_1.PriorityLevel.URGENT;
            case 'high':
                return models_1.PriorityLevel.HIGH;
            case 'medium':
                return models_1.PriorityLevel.MEDIUM;
            case 'low':
                return models_1.PriorityLevel.LOW;
            case 'standard':
                return models_1.PriorityLevel.STANDARD;
            default:
                return models_1.PriorityLevel.STANDARD;
        }
    }
}
exports.BinaryProtocolParser = BinaryProtocolParser;
//# sourceMappingURL=BinaryProtocolParser.js.map