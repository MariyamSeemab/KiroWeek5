...
exports.ReliabilityMetricsEngine = void 0;
const models_1 = require("../models");
class ReliabilityMetricsEngine {
    constructor() {
        this.BASELINE_ACCURACY = 99.99999; // 1 error in 16 million transactions
        this.HOP_WEIGHTS = {
            [models_1.HopType.BASE_HOP]: 0.1, // Collection to Local Station
            [models_1.HopType.TRANSIT_HOP]: 0.2, // Local Train to Sorting Hub
            [models_1.HopType.TRANSFER_HOP]: 0.5, // Dadar Interface Cross-line
            [models_1.HopType.FINAL_HOP]: 0.1 // Sorting Hub to Destination
        };
    ...
    calculateComplexityScore(route) {
    ...
    detectNetworkHops(route) {
        const hops = [];
        // Base Hop: Collection to Local Station (always present)
        hops.push({
            type: models_1.HopType.BASE_HOP,
            weight: this.HOP_WEIGHTS[models_1.HopType.BASE_HOP],
            description: 'Collection to Local Station',
            station: route.origin
        });
        // Transit Hop: Local Train to Sorting Hub (always through Dadar)
        hops.push({
            type: models_1.HopType.TRANSIT_HOP,
            weight: this.HOP_WEIGHTS[models_1.HopType.TRANSIT_HOP],
        ...
        if (this.isWesternToCentralCrossing(route)) {
            hops.push({
                type: models_1.HopType.TRANSFER_HOP,
                weight: this.HOP_WEIGHTS[models_1.HopType.TRANSFER_HOP],
        ...
        hops.push({
            type: models_1.HopType.FINAL_HOP,
            weight: this.HOP_WEIGHTS[models_1.HopType.FINAL_HOP],
    ...
    categorizeComplexity(score) {
        if (score >= 0.9) {
            return models_1.ComplexityRating.HIGH; // Cross-line Transfer / Multi-hub
        }
        else if (score >= 0.5) {
            return models_1.ComplexityRating.MEDIUM; // Inter-station Sorting
        }
        else {
            return models_1.ComplexityRating.LOW; // Direct Route
        }
    }
    // Check if route crosses from Western to Central lines
    isWesternToCentralCrossing(route) {
    ...
    calculateSystemConfidence(route, environmental) {
        ...
        if (environmental.isWesternToCentralCrossing) {
            ...
            degradationFactors.push({
                type: models_1.DegradationFactorType.DADAR_PENALTY,
                impact,
        ...
        if (this.isPeakHour(environmental.currentTime)) {
            ...
            confidence -= impact;
            degradationFactors.push({
                type: models_1.DegradationFactorType.PEAK_HOUR_JITTER,
                impact,
        ...
        if (environmental.monsoonMode) {
            ...
            if (totalRainImpact > 0) {
                ...
                degradationFactors.push({
                    type: models_1.DegradationFactorType.RAIN_VARIABLE,
    ...
    determineThresholdStatus(confidence) {
        let status;
        let message;
        let color;
        let actionRequired;
        if (confidence > 99.9) {
            status = models_1.ReliabilityThreshold.OPTIMAL;
            message = 'OPTIMAL ROUTE';
            color = '#32CD32'; // Green
            actionRequired = false;
        }
        else if (confidence >= 95.0) {
            status = models_1.ReliabilityThreshold.MONITORING;
            message = 'MONITORING ACTIVE (Delay Possible)';
            color = '#FFD700'; // Yellow
            actionRequired = false;
        }
        else {
            status = models_1.ReliabilityThreshold.CRITICAL;
    ...
    suggestAlternateRouting(route, confidence) {
        const alternatives = [];
        if (confidence < 99.0) {
    ...
    generateReliabilityMetrics(route, environmental) {
...
exports.ReliabilityMetricsEngine = ReliabilityMetricsEngine;
//# sourceMappingURL=ReliabilityMetricsEngine.js.map