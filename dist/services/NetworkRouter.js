...
const ReliabilityMetricsEngine_1 = require("./ReliabilityMetricsEngine");
const MumbaiSlangProcessor_1 = require("./MumbaiSlangProcessor");
class NetworkRouter {
    constructor(protocolKnowledgeBase) {
        this.protocolKB = protocolKnowledgeBase;
        this.reliabilityEngine = new ReliabilityMetricsEngine_1.ReliabilityMetricsEngine();
    }
    parseDeliveryMarker(marker) {
    ...
    extractMarkerComponents(marker) {
    ...
    validateMarkerComponents(marker) {
    ...
    generateRoutingPath(parsed) {
        ...
        const baseRoutingPath = {
            ...
            complexityScore: {}, // Will be populated by reliability engine
            systemConfidence: {}, // Will be populated by reliability engine
            reliabilityMetrics: {} // Will be populated by reliability engine
        };
        ...
        const reliabilityMetrics = this.reliabilityEngine.generateReliabilityMetrics(baseRoutingPath, environmentalFactors);
    ...
    calculateTiming(parsed) {
        ...
        if (parsed.color.priority === models_1.PriorityLevel.URGENT) {
    ...
    handleMumbaiSlang(input) {
        // Use the MumbaiSlangProcessor for comprehensive processing
        const slangProcessor = new MumbaiSlangProcessor_1.MumbaiSlangProcessor(this.protocolKB);
    ...
    createInvalidMarker(errorMessage) {
        return {
    ...
    createDefaultColorInfo(color) {
        return {
            name: color,
            priority: models_1.PriorityLevel.STANDARD,
            areaType: 'Unknown',
            hexCode: '#808080'
        };
    }
    createDefaultSymbolInfo(symbol) {
        return {
            shape: symbol,
            destinationType: models_1.DestinationType.INDUSTRIAL_ESTATE,
            description: 'Unknown symbol type'
        };
    }
    createDefaultStationInfo(code) {
        return {
            code,
    ...
    generateRouteSegments(destination) {
    ...
    determineTransportMode(destination) {
        // Determine transport mode based on destination area
        if (destination.area.includes('Western'))
            return 'Western Railway';
        if (destination.area.includes('Central'))
            return 'Central Railway';
        if (destination.area.includes('Harbour'))
            return 'Harbour Line';
